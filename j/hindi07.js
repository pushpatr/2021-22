...
//		weekday = 2; hr = 10; min = 45;
	...
	function checkClass(){
		if (classList[weekday].c == "") {
			...
			if(hr == (classList[weekday].t - 1) && (min < 45)){
				if (earlyF !==2) {
					earlyF = 1;
					showEarlyMsg();
				}
			}
			if( (( hr == (classList[weekday].t - 1) ) && (min > 44)) || ((hr == (classList[weekday].t)) && (min < 40)) ){
				if (showFormF !==2) {
					showFormF = 1;
					showFormMsg();
				}
	...
	const scriptURL = 'https://script.google.com/macros/s/AKfycbxHMrORPvG8u5_apCm82hAm8LfFTrwyo0AYNvDvFdPWtMtW1Zg8wBrGlXqL1kIHwMdM/exec';
